...
#Name output csv
csvname <- paste0("ERA5L_wind_5min",".csv")

# Set the working directory and read the CSV file
setwd("~/R/Bennett")
df <- read.csv('ERA5L_wind.csv')
...
df_long <- df %>% 
  gather(key = "location", value = "wind_speed", -datetime) %>% 
  complete(datetime = seq(min(datetime), max(datetime), by="5 min"), 
           nesting(location), fill = list(wind_speed=NA))

df_long$wind_speed <- approx(df_long$datetime, df_long$wind_speed, df_long$datetime)$y

# Reshape data to wide format
df_wide <- df_long %>% spread(key = "location", value = "wind_speed")
